...
# It's an OPTIONAL component: when it isn't installed, the core API
# (score_chunk, filter_chunks, probe, smart_chunk, embeddings, cache) still
# works, and the experimental names raise a clear ImportError only when called.
_EXPERIMENTAL_NAMES = (
...
except ImportError as _exp_exc:
    EXPERIMENTAL_AVAILABLE = False
    experimental = None
    _exp_reason = _exp_exc

    def _make_experimental_stub(name):
        def _stub(*args, **kwargs):
            raise ImportError(
                f"[pymrsf] '{name}' requires the experimental MRSF storage "
                f"backend, which is not available ({_exp_reason}). Core features "
                f"(score_chunk, filter_chunks, probe, smart_chunk) work without it."
            )
        _stub.__name__ = name
        return _stub

    for _name in _EXPERIMENTAL_NAMES:
        globals()[_name] = _make_experimental_stub(_name)
...
from .types import ScoreResult, ProbeResult
from .core import (
...
try:
    # Single source of truth: read the installed package version from metadata
    # (defined once in pyproject.toml). Falls back when run from a source tree.
    from importlib.metadata import version as _pkg_version, PackageNotFoundError
    try:
        __version__ = _pkg_version("pymrsf")
...
def configure(**kwargs) -> Config:
    ...
    Accepts any field name from Config. Changing `provider` is routed through
    set_provider() so the model/probe backend actually switches (not just the
    embeddings routing) — making Config the single source of truth.

    Note: import-time-only fields (model_path, n_ctx, n_gpu_layers,
    logit_precision) take effect on the next process / model load, since the
    local model is loaded once.
    ...
    if "provider" in kwargs:
        # Route through set_provider so core.PROVIDER / backend / model reset too.
        set_provider(_config.provider)
    return _config
