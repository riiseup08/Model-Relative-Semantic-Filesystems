...
def get_embedding_dim() -> int:
    ...
    # NOTE: do not hold _embed_dim_lock here — embed() acquires the same
    # (non-reentrant) lock internally to record the dimension, so wrapping this
    # call in the lock would self-deadlock on a cold cache.
    try:
        embed("test")  # populates _embed_dim_cache under the lock internally
    except Exception:
